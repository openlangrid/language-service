 ...
 * Copyright (C) 2009-2011 NICT Language Grid Project.
 *
...
extends AbstractTranslationService{
	 ...
	 * Constructor.
	 */
	public GoogleTranslation(ServiceContext context){
		super(context);
		init();
	}
	...
	public LanguagePair validateLanguagePair(
	...
	throws InvalidParameterException, ProcessFailedException
	{
		...
		result = deleteZeroWidthSpace(result);
		result = result.replaceAll("X\\u200b*X\\u200b*X\\u200b*(\\w+)\\u200b*X\\u200b*X\\u200b*X", "xxx$1xxx");
		return result;
	}
	...
	throws InvalidParameterException, ProcessFailedException
	{
		...
		for(int i=0; i < results.length; i++) {
			results[i] = deleteZeroWidthSpace(results[i]);
			results[i] = results[i].replaceAll("X\\u200b*X\\u200b*X\\u200b*(\\w+)\\u200b*X\\u200b*X\\u200b*X", "xxx$1xxx");
		}
		return results;
	}
	...
	private void init(){
		...
		referer = getInitParameterString(
				"googleTranslation.referer", ""
				);
	...
	/**
	 * 翻訳後に入ってしまうゼロ幅スペースを除去する。<br/>
	 * このメソッドは doTranslation と doMultistatementTranslation から呼び出される。
	 * @author Hitoshi Sugihara
	 * @param text 翻訳結果文
	 * @return ゼロ幅スペースを取り除いた文
	 */
	private String deleteZeroWidthSpace(String text) {
