...
/**
 * Webアプリケーションを用いた用例対訳サービスのiniファイルに対応
...
public class Webapp_ServiceDescription extends ServiceDescription {
	 ...
	 */
	public Webapp_ServiceDescription(InputStream inputStream)
	...
	/**
	 * 言語コードを受け取り、
	 * それを、langPathに含むような一致するような Webapp_Resource を返す
	 * 
	 * @param language 検索対象の言語
	 * @return languageをlangPathに含む最初のResource
	 */
	public Webapp_Resource getResources(Language language) {

		for (int i = 0; i < resources.length; i++) {

			if(resources[i].getLangPath().contains(language))
				return resources[i];
		}
		return null;
	}
