...
/**
 * Webアプリケーションを用いた用例対訳サービスのiniファイル.resourcesに対応
...
public class Webapp_Resource extends Resource {
	 ...
	 */
	public Webapp_Resource(JSONObject jObj, Webapp_Default defaultSource)
	throws ProcessFailedException {
		try{
			...
			//action*の切り出し
			if (jObj.has("action"))
				action = new Webapp_Action(jObj.getJSONObject("action"),defaultSource);
			else if(defaultSource!=null)
			...
			if (jObj.has("content"))
				content = 
					new Webapp_Content(jObj.getJSONObject("content"),defaultSource);
			else if(defaultSource!=null)
