/*
 * $Id: HTML_Resource.java 25906 2008-07-15 04:46:14Z murakami $
 *
...
/**
 * HTMLからの用例対訳のiniファイル.resourcesに対応
 * @author $Author: murakami $
 * @version $Revision: 25906 $
 */
public class HTML_Resource extends Resource {
	 ...
	 */
	public HTML_Resource(JSONObject jObj, HTML_Default defaultsource)
	throws ProcessFailedException {
		try {
			// 実際のキー毎の選択処理
			// (*は欠落を許すもの)
			...
			// contentの切り出し
			if (jObj.has("content")) {
				content = new HTML_Content(jObj.getJSONObject("content")
						,defaultsource);
			}else if (defaultsource!=null)
